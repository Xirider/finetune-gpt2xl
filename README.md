...
This guide explains how to finetune GPT2-xl with just one command of the Huggingface Transformers library. The DeepSpeed library and gradient checkpointing are used to lower the required GPU memory usage of the model, so that it can now fit on a single GPU.
